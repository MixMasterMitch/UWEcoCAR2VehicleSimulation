...
public class SimulationWindow {
	...
	public DynamicGraph graph = new DynamicGraph("title");
	...
	private final JButton unstepButton = new JButton("|<");
	...
	public SimulationWindow(DiscreteStepSimulator s) {
		...
		backButton.addActionListener(new ActionListener() {
			@Override
			public void actionPerformed(ActionEvent event) {
				saveAndResetButton.doClick();
			}
		});
		
		// Undo one step, not implemented yet
		/*
		unstepButton.addActionListener(new ActionListener() {
			@Override
			public void actionPerformed(ActionEvent event) {
		...
		*/
		...
		mainWindow.add(graph.getContentPane(), BorderLayout.NORTH);
	...
	private void setGraph() {
		...
		switch (index) {
			case 0 : this.graph.setData(vehicle.getVelocityRecorder());
				break;
			case 1 : this.graph.setData(vehicle.getDistanceTravelledRecorder());
				break;
			case 2 : this.graph.setData(vehicle.getAccelerationRecorder());
				break;
			default: this.graph.setData(vehicle.getVelocityRecorder());
		}
	}
	...
	private void checkButtons() {
		...
		if (simulator.isStarted() && !backButton.isEnabled()) {
            // if one of the is disable
        	backButton.setEnabled(true);
			//unstepButton.setEnabled(true);
		} else if (!simulator.isStarted() || (simulator.isStarted() && !backButton.isEnabled()) ){
			backButton.setEnabled(false);
		}
	}
}